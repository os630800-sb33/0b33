...
| 1006 | `BatchTooLarge` | Auth | `admin.rs`, `lib.rs`, `subscription.rs`, `test.rs`, `test_bulk_admin_ops.rs` | Reduce batch size to ≤ BATCH_MAX_SIZE entries and retry. | — |
...
| 4005 | `Replay` | State Transition | `admin.rs`, `charge_core.rs`, `coupon.rs`, `test.rs`, `test_coupon.rs`, `test_idempotency_keys.rs`, `test_nonce_domains.rs`, `test_recovery.rs`, `test_reentrancy_invariants.rs` | Treat as idempotent duplicate; do not retry with a new key for the same action. | — |
...
| 4007 | `EmergencyStopActive` | State Transition | `charge_core.rs`, `lib.rs`, `test.rs`, `test_cross_contract_reentrancy.rs`, `test_emergency_stop_lifetime_caps.rs`, `test_emergency_stop_matrix.rs`, `test_emergency_stop_view_surface.rs`, `test_operator.rs`, `test_reentrancy_invariants.rs` | Pause writes; poll get_emergency_stop_status and retry after admin clears stop. | EmergencyStopDisabledEvent |
...
| 7004 | `MerchantNotApproved` | Merchant Config | `merchant.rs`, `test_merchant_whitelist.rs` | Request admin approval or disable whitelist mode via set_merchant_whitelist_mode. | — |
...
| 9102 | `SchemaVersionMismatch` | Schema Migration | `admin.rs`, `test.rs`, `test_config_migration.rs` | Migration rejected; deploy compatible binary. | — |
